import { PrismaClient, Channel, CustomerStatus, DealStatus, MessageSender } from '@prisma/client';
...
async function main() {
  ...
  // Create customers with tags, deals, and conversations
  for (let i = 0; i < customersData.length; i++) {
    ...
    for (const tag of data.tags) {
      await prisma.customerTag.upsert({
        where: { customerId_name: { customerId: customer.id, name: tag } },
        update: {},
        create: {
          name: tag,
    ...
    const messages: { text: string; sender: MessageSender; minutesAgo: number }[] = [
      { text: `Hi, I'm interested in ${data.dealName}`, sender: 'customer', minutesAgo: 60 },
      { text: 'Hello! Thanks for reaching out. How can I help you today?', sender: 'user', minutesAgo: 55 },
      { text: 'I wanted to know more about the pricing and features.', sender: 'customer', minutesAgo: 50 },
  ...
  console.log(`Created ${customersData.length} customers with deals and conversations`);
  ...
  for (const log of updateLogs) {
    const existingLog = await prisma.updateLog.findFirst({
      where: { version: log.version },
    });

    if (!existingLog) {
      const createdLog = await prisma.updateLog.create({
        data: {
          version: log.version,
          date: log.date,
      ...
      for (const change of log.changes) {
        await prisma.updateLogChange.create({
          data: {
            change,
  ...
  for (const email of emails) {
    const existingEmail = await prisma.email.findFirst({
      where: { subject: email.subject, fromEmail: email.fromEmail },
    });
